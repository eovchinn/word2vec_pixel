...
from numpy import exp, log, dot, zeros, outer, random, dtype, float32 as REAL,\
    ...
    ndarray, empty, sum as np_sum, prod, ones, ascontiguousarray, vstack, logaddexp, random
...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(
            self, corpus=None,train=None, dict_size=None, ind_name=None, size=100, alpha=0.025, window=5, min_count=5,
        ...
        if ind_name is not None and train is not None:
            self.build_vocab_pix(train, dict_size, ind_name, fdr=fdr, int_per=int_per, pix_per=pix_per, quan=quan)
    ...
    def build_vocab_pix(self, ds_dir, dict_size, ind_name, min_count=None, sample=None, fdr=None, int_per=None, pix_per=None, quan=None):
        logger.info("Building vocabulary for %s from %s", ind_name, ds_dir)

        self.corpus_count = 0
        """Calculate word frequences and number of pixels given as examples."""
        ind_freq = dict.fromkeys(range(0, dict_size),0)

        for f in os.listdir(ds_dir):
            ds_df = pd.read_msgpack(os.path.join(ds_dir,f))

            #logger.info("ds_df pixel size %i ", len(ds_df[['x', 'y']].drop_duplicates().index))
            # filter out rows based on fdr
            ...
            freq_counts = dict(filt_df[ind_name].value_counts())
            for i in freq_counts:
                ind_freq[i] += freq_counts[i]

            # how many pixels will be taken from each dataset
            self.corpus_count += round(len(filt_df[['x', 'y']].drop_duplicates().index) * pix_per)

        """Filter out unfrequent words, frequent words, set up word probabilities."""
        min_count = min_count or self.min_count
        sample = sample or self.sample

        ind_freq_res = {i: f for i,f in ind_freq.items() if f>= min_count}
        retain_total = sum(itervalues(ind_freq_res))

        #self.total_words = sum(itervalues(ind_freq_res))
        ...
        for i in ind_freq_res:
            v = ind_freq_res[i]
            word = str(i) # intead of ion or formula name we just keep it's pandas id to make processing faster, it has to be a string
        ...
        def job_producer():
            ...
            for word_list in corpus:
                ...
                if batch_size + list_length <= self.batch_words:
                    ...
                    job_batch, batch_size = [word_list], list_length
...
class PixelCorpus(object):
    def __init__(self, ds_dir, ind_name, fdr_thresh, pix_per, int_per, window, quan):
        self.f = fdr_thresh
        self.ds_dir = ds_dir
        self.p = pix_per
        self.i = int_per
    ...
    def __iter__(self):
        for f in os.listdir(self.ds_dir):
            ds_df = pd.read_msgpack(os.path.join(self.ds_dir,f))

            # filter out rows based on fdr threshold self.f
            ds_df = ds_df[ds_df.fdr < self.f]
            # filter out rows based on intensity percentage threshold self.i and quantile parameter self.q
            first = True 
            for _, ion_rows in ds_df.groupby('ion_ind'):
                # remove hot spots
                perc = percentile(ion_rows.int.tolist(), 99.)
            ...
            for _, c_row in sampled_coord_df.iterrows():
                ...
                # depending on how many times an ion/or formula (depending on self.ind_name parameter)
                # occurs in the window, yield it
                exp_inds = []
                ind_counts = dict(window_rows[self.ind_name].value_counts())
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ind_name", help="Calculate model for ion or formula", default='ion')
    parser.add_argument("-fdr", help="FDR threshold", type=int, default=51)
    ...
    corpus = PixelCorpus(args.train, args.ind_name, args.fdr, args.pix_per, args.int_per, args.window, args.quan)
    ...
    model = Word2Vec(
        corpus=corpus, train=args.train, dict_size=dict_size, ind_name=args.ind_name, 
